...
async def _decompose_once(normalized_source: str) -> list[RawStep]:
    llm = get_llm(max_tokens=DECOMPOSE_MAX_TOKENS).bind_tools(
    ...
    if response.response_metadata.get("stop_reason") == "max_tokens":
        # The tool call's JSON was cut off mid-generation — whatever was
        # salvaged (often empty/partial args) is not a real decomposition.
        # Long, multi-proof inputs are the common trigger; say so
        # specifically rather than the misleading "returned no steps" a
        # truncated call would otherwise produce. Not retried — same input,
        # same size, same truncation point.
    ...
    if not isinstance(raw_steps, list) or not raw_steps:
        ...
        # said. Log the full tool call args and any accompanying text block
        ...
        _logger.warning(
            "Empty decomposition steps. stop_reason=%r tool_call.args=%r text_blocks=%r",
...
async def decompose(normalized_source: str) -> list[RawStep]:
    ...
    for _ in range(MAX_EMPTY_RETRIES + 1):
        try:
            return await _decompose_once(normalized_source)
